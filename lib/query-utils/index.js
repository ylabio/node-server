      ...
      if (typeof type.kind === 'function') {
        ...
        switch (type.kind.toLowerCase()) {
          ...
          case 'between':
            ...
            if (values.length === 2) {
              $in.push({
                  $gte: this.number(values[0]),
                  $lte: this.number(values[1])
                }
              );
            } else {
              $in.push({$eq: this.number(values[0])});
            }
            break;
          case 'between-age':
            values = value.split(';');
            ...
            break;
          case 'gt':
            $in.push({$gt: this.number(value)});
            break;
          case 'lt':
            $in.push({$lt: this.number(value)});
            break;
          case 'gte':
            $in.push({$gte: this.number(value)});
            break;
          case 'lte':
            $in.push({$lte: this.number(value)});
            break;
          case 'const':
          case 'eq':
          default:
